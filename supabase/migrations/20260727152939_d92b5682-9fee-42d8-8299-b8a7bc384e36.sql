-- Reemplaza la función privada de reactivación para incluir reconstrucción
-- determinística del ciclo cuando no exista una captura estructurada previa.
-- Se conserva la firma, los permisos y la transaccionalidad.
...
BEGIN
  ...
  IF _tipo_caso = 'entrante' THEN
    ...
    v_initial_state := NULL; -- entrantes: no se reconstruye (múltiples estados iniciales posibles)
  ELSIF _tipo_caso = 'remision' THEN
    ...
    -- Tipos de seguimiento que SÍ representan una transición del estado principal.
    v_mutating_types := ARRAY[
      ...
      'REVISIÓN AUTORIZACIÓN ESTANCIA (CANCELACIÓN)'
    ];
    -- Valores de estado_solicitud que denotan una transición estructurada.
  ...
  -- Bloquear registro y leer estado + updated_at + created_at
  IF _tipo_caso = 'entrante' THEN
  ...
  -- FUENTE 1: captura directa del disparador de cancelación
  ...
  ELSE
    ...
    -- FUENTE 3: reconstrucción determinística (solo remision / domiciliario)
    IF v_initial_state IS NOT NULL THEN
      v_evidencia_creacion := v_case_created_at IS NOT NULL;

      -- Contar eventos estructurados que sí cambian el estado principal
      SELECT count(*)
