DROP FUNCTION IF EXISTS public.calcular_indicadores_mes(integer, integer);

CREATE OR REPLACE FUNCTION public.calcular_indicadores_mes(_year integer, _month integer)
 RETURNS TABLE(out_codigo text, out_tipo text, out_num numeric, out_den numeric, out_res numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_den > 0 THEN
    ...
    VALUES
      (v_ind4, v_periodo, v_ini, v_fin - 1, v_tipo, 'casos_entrantes+historicos_casos',
       ...
       v_den::int, 'v1.1', now(),
       'Hibrido: casos_entrantes (con snapshot EAPB) + historicos_casos seccion=entrante.')
    ...
    out_codigo := 'IND-INST-04'; out_tipo := v_tipo; out_num := v_num; out_den := v_den; out_res := v_res;
    RETURN NEXT;
  END IF;
  ...
  IF v_den > 0 THEN
    ...
    VALUES
      (v_ind3, v_periodo, v_ini, v_fin - 1, v_tipo, 'remisiones+historicos_casos',
       v_num, v_den, v_res, v_meta, '%',
       v_den::int, 'v1.1', now(),
       'Hibrido: remisiones + historicos_casos seccion=saliente. REMITIDO=aceptado.')
    ON CONFLICT (indicador_id, periodo, tipo_medicion, regla_version) DO UPDATE
      SET numerador_valor = EXCLUDED.numerador_valor,
          denominador_valor = EXCLUDED.denominador_valor,
          resultado = EXCLUDED.resultado,
    ...
    out_codigo := 'IND-INST-03'; out_tipo := v_tipo; out_num := v_num; out_den := v_den; out_res := v_res;
    RETURN NEXT;
  END IF;
  ...
  IF v_evaluables > 0 THEN
    ...
    out_codigo := 'IND-INST-01'; out_tipo := v_tipo; out_num := round(v_num,2); out_den := v_den; out_res := v_res;
    RETURN NEXT;
  END IF;
  ...
  IF v_evaluables > 0 THEN
    ...
    out_codigo := 'IND-INST-02'; out_tipo := v_tipo; out_num := round(v_num,2); out_den := v_den; out_res := v_res;
    RETURN NEXT;
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.calcular_indicadores_mes(integer, integer) FROM PUBLIC, anon, authenticated;
