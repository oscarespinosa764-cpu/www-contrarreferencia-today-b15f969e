...
BEGIN
  ...
  -- ---------------- Indicador 4: Proporción entrantes aceptadas (híbrido) ----------------
  WITH fuente AS (
  ...
  IF v_den > 0 THEN
    ...
    VALUES
      (v_ind4, v_periodo, v_ini, v_fin - 1, v_tipo, 'casos_entrantes+historicos_casos',
       ...
       v_den::int, 'v1.1', now(),
       'Hibrido: casos_entrantes (con snapshot EAPB contratada) + historicos_casos seccion=entrante. ACEPT/INGRES/REMIT = aceptado; NO ACEPTADO/CANCEL/NEGAD/SUSPEND/RECHAZ = no aceptado.')
  ...
  -- ---------------- Indicador 3: Proporción salientes aceptadas (híbrido) ----------------
  WITH fuente AS (
    SELECT upper(coalesce(estado,'')) AS est
  ...
  IF v_den > 0 THEN
    ...
    VALUES
      (v_ind3, v_periodo, v_ini, v_fin - 1, v_tipo, 'remisiones+historicos_casos',
       v_num, v_den, v_res, v_meta, '%',
       v_den::int, 'v1.1', now(),
       'Hibrido: remisiones + historicos_casos seccion=saliente. REMITIDO=aceptado; SUSPENDIDO/CANCELADO/NEGADO/RECHAZADO=no aceptado.')
    ON CONFLICT (indicador_id, periodo, tipo_medicion, regla_version) DO UPDATE
      SET numerador_valor = EXCLUDED.numerador_valor,
          denominador_valor = EXCLUDED.denominador_valor,
          resultado = EXCLUDED.resultado,
  ...
  -- Solo casos operativos con timestamps reales. NO aplica a historicos_casos.
  WITH tiempos AS (
    SELECT ce.id,
  ...
  IF v_evaluables > 0 THEN
    ...
    VALUES
      (v_ind1, v_periodo, v_ini, v_fin - 1, v_tipo, 'casos_entrantes+seguimientos',
       ...
       'Solo casos con timestamp real de recepcion y primera respuesta. No aplica a historicos importados sin hora.')
    ON CONFLICT (indicador_id, periodo, tipo_medicion, regla_version) DO UPDATE
      SET numerador_valor = EXCLUDED.numerador_valor,
          denominador_valor = EXCLUDED.denominador_valor,
          resultado = EXCLUDED.resultado,
