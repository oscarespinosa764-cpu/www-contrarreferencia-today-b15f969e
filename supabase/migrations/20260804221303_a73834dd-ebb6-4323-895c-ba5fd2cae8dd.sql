create or replace function public.historial_listado(
  _module text, _start timestamptz, _end timestamptz,
  _tipo text default null, _estado text default null, _sede text default null,
  _servicio text default null, _documento text default null, _term text default null,
  _subtype text default null, _page integer default 1, _page_size integer default 25)
returns jsonb
language plpgsql
stable
...
begin
  ...
  with base as (
    ...
    where u.modulo = any(_mods)
      ...
      and (_estado is null
           or (case when upper(_estado) = 'CERRADO'
                 then u.estado_txt ~* '(COMPLET|CERRAD|CULMIN)'
                 else u.estado_txt ilike '%'||_estado||'%' end))
...
revoke all on function public.historial_listado(text,timestamptz,timestamptz,text,text,text,text,text,text,text,int,int) from public, anon;
grant execute on function public.historial_listado(text,timestamptz,timestamptz,text,text,text,text,text,text,text,int,int) to authenticated, service_role;