ALTER TABLE public.catalogos
  ADD COLUMN IF NOT EXISTS evolucion_por_correo boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.catalogos.evolucion_por_correo IS
  'EAPB/ERP: regla operativa. TRUE = la evolución diaria se realiza por CORREO ELECTRONICO. No depende de eapb_correo_radicacion.';
COMMENT ON COLUMN public.catalogos.eapb_correo_radicacion IS
  'LEGADO (Fase 5E C.5): dato de contacto. No participa en ninguna regla operativa de Evolución Diaria. Se conserva hasta el módulo RED & DISPONIBILIDAD.';

-- Config canónica: dos indicadores booleanos, sin direcciones de correo.
DROP FUNCTION IF EXISTS private.eapb_catalogo_cfg(text);
CREATE FUNCTION private.eapb_catalogo_cfg(_txt text)
RETURNS TABLE(n integer, correo boolean, plataforma boolean, activo boolean, canonica text)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path TO ''
...
DECLARE
  v_n int := 0;
BEGIN
  n := 0; correo := NULL; plataforma := NULL; activo := NULL; canonica := NULL;
  ...
  IF v_n = 1 THEN
    SELECT 1, coalesce(c.evolucion_por_correo,false), coalesce(c.seguimientos_en_plataforma,false), true, c.valor
      INTO n, correo, plataforma, activo, canonica
      FROM public.catalogos c
     WHERE c.tipo = 'EAPB' AND coalesce(c.activo,false) = true
       AND private.norm_txt(c.valor) = private.norm_txt(_txt);
    RETURN NEXT; RETURN;
  ELSIF v_n > 1 THEN
    n := v_n; RETURN NEXT; RETURN;
  END IF;
...
ALTER FUNCTION private.eapb_catalogo_cfg(text) OWNER TO postgres;
...
BEGIN
  ...
  IF coalesce(v_n,0) = 0 THEN
    RAISE EXCEPTION 'No fue posible validar la configuración operativa de evolución de la EAPB/ERP. Solo se permite un canal.'
      USING ERRCODE = 'check_violation';
  ELSIF v_n > 1 THEN
  ...
  IF coalesce(v_correo,false) <> true OR coalesce(v_plataforma,false) <> true THEN
    RAISE EXCEPTION '%', v_msg_dual USING ERRCODE = 'check_violation';
  END IF;
