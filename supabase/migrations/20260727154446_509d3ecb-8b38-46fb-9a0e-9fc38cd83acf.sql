CREATE OR REPLACE FUNCTION private.reactivar_caso_cancelado_admin(_requester uuid, _tipo_caso text, _caso_id uuid, _motivo_reactivacion text, _updated_at_esperado timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  IF _tipo_caso = 'entrante' THEN
    SELECT estado, updated_at, created_at, archivado
      INTO v_current_state, v_current_updated_at, v_case_created_at, v_current_archivado
  ...
  ELSIF _tipo_caso = 'domiciliario' THEN
    SELECT estado_ciclo, updated_at, created_at, archivado
      INTO v_current_state, v_current_updated_at, v_case_created_at, v_current_archivado
  ...
  -- Restaura estado operativo Y desarchiva para que el caso reingrese al
  -- Dashboard Operativo (que filtra por archivado = false). La cancelación
  -- histórica permanece preservada en seguimientos/auditoría.
  IF _tipo_caso = 'entrante' THEN
    UPDATE public.casos_entrantes
       SET estado = v_previous_state, archivado = false, updated_at = now()
     WHERE id = _caso_id;
  ELSIF _tipo_caso = 'remision' THEN
  ...
  ELSIF _tipo_caso = 'domiciliario' THEN
    UPDATE public.domiciliarios
       SET estado_ciclo = v_previous_state, archivado = false, updated_at = now()
     WHERE id = _caso_id;
  END IF;
  ...
  INSERT INTO public.seguimientos (
    ...
    jsonb_build_object(
      ...
      'archivado_previo', v_current_archivado,
      'archivado_restaurado', false,
...
-- Corrección puntual del caso 2006973: reactivación previa restauró estado
-- pero no desarchivó (versión anterior de la función). Se desarchiva sin
-- crear un segundo evento REACTIVACION_ADMINISTRATIVA para no duplicar
-- historial; queda constancia en audit_logs como corrección técnica.
DO $mig$
DECLARE
  v_id uuid;
  v_admin uuid;
  v_prev boolean;
BEGIN
  SELECT id, archivado INTO v_id, v_prev
    FROM public.remisiones
   WHERE codigo_radicacion = '2006973'
   LIMIT 1;

  IF v_id IS NOT NULL AND COALESCE(v_prev, false) = true THEN
    UPDATE public.remisiones
       SET archivado = false, updated_at = now()
     WHERE id = v_id;

    -- Atribuir la corrección técnica al admin que reactivó el caso
    SELECT created_by INTO v_admin
      FROM public.seguimientos
     WHERE caso_id = v_id
       AND tipo_caso = 'remision'
       AND tipo_seguimiento = 'REACTIVACION_ADMINISTRATIVA'
     ORDER BY created_at DESC
     LIMIT 1;
    ...
    IF v_admin IS NOT NULL THEN
      PERFORM public.registrar_auditoria_srv(
        ...
        jsonb_build_object(
          'tipo_caso', 'remision',
          'archivado_previo', true,
          'archivado_restaurado', false,
          'nota', 'Corrección técnica: reactivación previa restauró estado pero no desarchivó; se ajusta sin generar segunda REACTIVACION_ADMINISTRATIVA.'
        ),
        NULL, NULL
      );
    END IF;
