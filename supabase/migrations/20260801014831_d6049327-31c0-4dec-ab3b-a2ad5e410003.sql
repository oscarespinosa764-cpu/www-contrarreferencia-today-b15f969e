ALTER TABLE public.catalogos
  ADD COLUMN IF NOT EXISTS nueva_eps_rnc_correo_adicional boolean NOT NULL DEFAULT false;

UPDATE public.catalogos
   SET nueva_eps_rnc_correo_adicional = true
 WHERE tipo = 'ESPECIALIDAD'
   AND private.norm_txt(valor) = 'ORTOPEDIA Y TRAUMATOLOGIA';

CREATE OR REPLACE FUNCTION private.esp_requiere_correo_adicional(_esp text[])
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
  ...
  SELECT EXISTS (
    SELECT 1
     ...
     WHERE c.tipo = 'ESPECIALIDAD'
       ...
       AND coalesce(c.nueva_eps_rnc_correo_adicional, false) = true
       AND private.norm_txt(c.valor) = ANY (
             SELECT private.norm_txt(e) FROM unnest(coalesce(_esp, '{}'::text[])) e
           )
  )
$$;

REVOKE ALL ON FUNCTION private.esp_requiere_correo_adicional(text[]) FROM PUBLIC;
...
BEGIN
  ...
  -- Regla global: plataforma declarada como no funcional NUNCA puede quedar
  -- registrada como realizada (aplica con o sin excepción).
  ...
  -- Regla configurable por especialidad (catálogo, NUNCA payload del cliente).
  v_doble := private.esp_requiere_correo_adicional(v_esp_req);
  ...
  IF v_excepcion IS NOT NULL
    ...
    RAISE EXCEPTION 'En este caso (NUEVA EPS · RED NO CONTRATADA) solo aplica un canal de evolución: %',
      CASE WHEN v_variante = 'SOLO_PLATAFORMA' THEN 'PLATAFORMA WEB' ELSE 'CORREO ELECTRONICO' END
      USING ERRCODE = 'check_violation';
  END IF;
  ...
  -- Blindaje: plataforma pendiente por falla nunca produce EVOLUCIONADO.
  IF v_falla THEN
    v_estado := CASE WHEN v_estado = 'SIN_EVOLUCIONAR' THEN v_estado ELSE 'EVOLUCION_PARCIAL' END;
  END IF;
