-- B2: aceptar actor validado por la server function. Al invocarse vía
-- service_role, auth.uid() es NULL; el actor real lo aporta la capa server
-- después de requireSupabaseAuth + is_active_member.
DROP FUNCTION IF EXISTS public.registrar_cambio_unidad_ri(uuid, text, text, text, text);
...
BEGIN
  ...
  -- Bloquea la fila y valida estado activo (protege contra concurrencia).
  ...
  -- Cama actual = último CAMBIO DE UNIDAD registrado (referencia_interna no
  -- tiene columna cama; se mantiene estructurada en el seguimiento).
...
-- La RPC sigue restringida al servidor. anon/authenticated no pueden invocarla.
REVOKE ALL ON FUNCTION public.registrar_cambio_unidad_ri(uuid, text, text, text, text, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.registrar_cambio_unidad_ri(uuid, text, text, text, text, uuid) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.registrar_cambio_unidad_ri(uuid, text, text, text, text, uuid) TO service_role;