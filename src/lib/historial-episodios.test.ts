...
describe("cambios de estado registrados", () => {
  const ce = (ant: string, nue: string, t: string, actor = "ANA") =>
    ({ ...actuacionCambioEstado({ caso_id: "x", estado_anterior: ant, estado_nuevo: nue, actor_nombre: actor, created_at: t }) });
  it("2 cambios → 3 fases, cambio es primera actuación y gestores propios", () => {
    const acts = [
      { accion: "SEG", estado: "NO ACEPTA", funcionario: "LUIS", _orden: Date.parse("2026-01-01T01:00Z") },
    ...
    expect(f.map((x) => x.estado)).toEqual(["PENDIENTE ACEPTACION", "ACEPTADO", "AMBULANCIA COORDINADA"]);
    expect(f[1].actuaciones[0].accion).toBe("CAMBIO DE ESTADO");
    expect(f[1].actuaciones.map((a) => (a as { funcionario: string }).funcionario)).toEqual(["ANA", "PEDRO"]);
  ...
  it("entrantes: clasificación no abre fase, ingreso sí", () => {
    const f = derivarFases([a("ACEPTACIÓN", "ACEPTADO", 1), a("INGRESO CONFIRMADO", "", 2)], {
      estadoActual: "INGRESO CONFIRMADO", inicio: 0, transiciones: TRANSICIONES.entrantes,
    });
