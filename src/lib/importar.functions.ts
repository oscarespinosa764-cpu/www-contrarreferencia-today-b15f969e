...
export const exportarMasivo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (adminErr || !esAdmin) {
      return { ok: false, columnas: def.columnas, filas: [] as Record<string, unknown>[], error: "Acción reservada a coordinación." };
    }
    ...
    if (error) {
      ...
      return { ok: false, columnas: def.columnas, filas: [] as Record<string, unknown>[], error: error.message };
    }
