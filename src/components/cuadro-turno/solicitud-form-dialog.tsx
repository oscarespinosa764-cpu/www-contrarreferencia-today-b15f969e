...
function FraccionEditor({
  frag, index, funcionarios, shiftTypes, onChange, onRemove,
}: {
  frag: ReturnFragment;
  index: number;
  ...
  const setReceiver = (nombre: string) => {
    ...
    onChange({ ...frag, receiver_name: nombre, receiver_id: f?.userId ?? null, receiver_role: f?.cargo ?? null });
  };

  return (
    <div className="rounded-md border p-3">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-xs font-semibold uppercase text-muted-foreground">Fracción {index + 1}</span>
        {onRemove && (
          <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-rose-600" onClick={onRemove}>
      ...
      <div className="grid grid-cols-2 gap-3">
          ...
          <Select value={frag.receiver_name || ""} onValueChange={setReceiver}>
            ...
            <SelectContent>
              {funcionarios.length === 0 ? (
                <SelectItem value="__none" disabled>Sin funcionarios</SelectItem>
              ) : (
        ...
        <div><Label className="text-xs">Cargo</Label><Input value={frag.receiver_role || ""} readOnly className="bg-muted/40" placeholder="Automático" /></div>
        <div><Label className="text-xs">Fecha de devolución</Label><Input type="date" value={frag.return_date || ""} onChange={(e) => onChange({ ...frag, return_date: e.target.value })} /></div>
        <div>
          ...
          <div className="flex h-9 items-center gap-2 rounded-md border bg-muted/40 px-2 text-xs">
            {info ? (
              <>
                ...
                <span className="ml-auto rounded bg-vitalis-blue/15 px-1.5 py-0.5 font-bold text-vitalis-blue">{info.code}</span>
              </>
            ) : (
              <span className="text-muted-foreground">{frag.receiver_name && frag.return_date ? "Sin turno ese día" : "Automático"}</span>
            )}
          </div>
        </div>
        <div><Label className="text-xs">Hora inicial de devolución</Label><TimeField value={frag.start_time || ""} onChange={(v) => onChange({ ...frag, start_time: v, minutes: minutosEntreHoras(v, frag.end_time) })} /></div>
        <div><Label className="text-xs">Hora final de devolución</Label><TimeField value={frag.end_time || ""} onChange={(v) => onChange({ ...frag, end_time: v, minutes: minutosEntreHoras(frag.start_time, v) })} /></div>
        <div className="col-span-2 flex items-center gap-2 text-xs">
          <span className="rounded bg-muted px-2 py-1 font-semibold">Total: {minutosAHoras(minutos)}</span>
          <span className="text-muted-foreground">El horario puede ajustarse cuando la devolución corresponda a una fracción del turno.</span>
        </div>
        <div className="col-span-2"><Label className="text-xs">Observaciones</Label><Input value={frag.notes || ""} onChange={(e) => onChange({ ...frag, notes: e.target.value })} /></div>
      </div>
    </div>
  );
}
...
export function SolicitudFormDialog({
  ...
  const [uso, setUso] = useState<{ solicitudes: number; coberturas: number; pendientes: number; total: number } | null>(null);
  ...
  useEffect(() => {
    if (esCambio || !user || !startDate) { setTurnoSolicitanteCode(null); return; }
    let cancel = false;
    (async () => {
      setBuscandoTurno(true);
      const t = await buscarTurnoProgramado({ userId: user.id, fullName: perfil?.nombre, fecha: startDate });
      if (cancel) return;
  ...
  const handleMotivo = (v: string) => {
    ...
    if (!cambio) {
      ...
      if (!rec) {
        ...
        setRemunerado(false);
        setReempNombre(""); setReempCargo(""); setReempUserId(null);
        setFracciones([]); setFraccionado(false);
      }
    }
  };
  ...
  const handleReemplazo = (nombre: string) => {
    ...
    setFracciones((prev) => prev.map((fr, i) => (i === 0 && !fr.receiver_name
      ? { ...fr, receiver_name: nombre, receiver_id: f?.userId ?? null, receiver_role: f?.cargo ?? null }
      : fr)));
  };
  ...
  // ---- Cálculo de horas ----
  const minutosSolicitados = useMemo(() => minutosEntreHoras(startTime, endTime), [startTime, endTime]);
  ...
  const descuadre = recupera && minutosSolicitados > 0 && minutosProgramados > 0 && minutosProgramados !== minutosSolicitados;
  ...
  async function excepcionDisponible(y: number, m: number): Promise<string | null> {
    ...
    const { data } = await supabase
      ...
      .select("id")
      .eq("user_id", user.id).eq("year", y).eq("month", m)
      .eq("status", "APROBADA").eq("usage_status", "DISPONIBLE")
      .maybeSingle();
    return data?.id ?? null;
  }
  ...
  const enviarExcepcion = async () => {
    ...
    try {
      const { data: prev } = await supabase
        .from("shift_monthly_exceptions")
        .select("id").eq("user_id", user.id).eq("year", mm.y).eq("month", mm.m)
        .eq("status", "PENDIENTE").maybeSingle();
      if (prev) { toast.error("Ya existe una autorización excepcional pendiente para este mes."); setExcSaving(false); return; }
      const { error } = await supabase.from("shift_monthly_exceptions").insert({
        user_id: user.id, user_name: perfil?.nombre, user_role: perfil?.cargo,
        year: mm.y, month: mm.m, request_type: esCambio ? "cambio_turno" : "permiso",
        reason: excMotivo, counts: uso ?? {}, status: "PENDIENTE", usage_status: "DISPONIBLE",
      });
      if (error) throw error;
      registrarAuditoria({ data: { accion: "EXCEPCION_ENVIADA", modulo: "cuadro_turno", tabla: "shift_monthly_exceptions", resultado: "exito" } }).catch(() => {});
      ...
      setLimiteOpen(false); setExcMode("aviso"); setExcMotivo("");
    } catch (e) {
      console.error(e);
  ...
  const submit = async () => {
    ...
    if (!esCambio && recupera && fracciones.some((f) => !f.receiver_name || !f.return_date || !f.start_time || !f.end_time))
      return toast.error("Completa los datos de la devolución del tiempo.");
    if (motivoExento && !soporte) return toast.error("Este permiso requiere un soporte obligatorio.");
    if (descuadre && !detalle.trim())
      return toast.error("Las horas programadas no coinciden con las solicitadas. Justifícalo en 'Especifique motivo'.");
    ...
    if (!motivoExento) {
      const mm = mesAfectado();
      if (mm) {
        const { data: usoData } = await supabase.rpc("shift_monthly_usage", { _user_id: user.id, _year: mm.y, _month: mm.m });
        const row = Array.isArray(usoData) ? usoData[0] : usoData;
        const total = (row?.solicitudes ?? 0) + (row?.coberturas ?? 0);
        setUso({ solicitudes: row?.solicitudes ?? 0, coberturas: row?.coberturas ?? 0, pendientes: row?.pendientes ?? 0, total });
        if (total >= LIMITE_MENSUAL) {
          usarExcepcionId = await excepcionDisponible(mm.y, mm.m);
          if (!usarExcepcionId) { setExcMode("aviso"); setLimiteOpen(true); return; }
        }
      }
    }
    ...
    try {
      ...
      if (usarExcepcionId) {
        await supabase.from("shift_monthly_exceptions")
          .update({ usage_status: "UTILIZADA", used_request_id: req.id, used_at: new Date().toISOString() })
  ...
  const resetForm = () => {
    ...
    setRemunerado(false); setReempNombre(""); setReempCargo(""); setReempUserId(null); setDetalle("");
    setObservaciones(""); setOrigFecha(""); setOrigTurno(""); setNuevaFecha("");
    setNuevoTurno(""); setCompanero(""); setConfirmo(false); setEsCambio(false);
    setFraccionado(false); setFracciones([]); setTurnoSolicitanteCode(null);
    setSoporte(null); setUso(null); setExcMotivo(""); setExcMode("aviso");
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[92vh] max-w-2xl flex-col overflow-hidden">
        ...
        <div className="scrollbar-invisible flex-1 space-y-4 overflow-y-auto pr-0.5 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select value={motivo} onValueChange={handleMotivo}>
                ...
                <SelectContent>
                  {motivos.map((m) => (
                    <SelectItem key={m.valor} value={m.valor}>{m.valor}</SelectItem>
                  ))}
          ...
          {esCambio ? (
            ...
            <fieldset className="rounded-md border p-3">
              ...
              <div className="mt-3 rounded-md border border-vitalis-blue/30 bg-vitalis-blue/5 p-2.5">
                <p className="text-[11px] font-semibold uppercase text-vitalis-blue">Turno programado del solicitante</p>
                {buscandoTurno ? (
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                    ...
                    {turnoSolInfo.hours ? <span className="text-muted-foreground">· {turnoSolInfo.hours} h</span> : null}
                    <span className="rounded bg-vitalis-blue/15 px-1.5 py-0.5 font-bold text-vitalis-blue">Código {turnoSolInfo.code}</span>
                  </div>
                ) : (
                  <p className="mt-1 text-xs text-muted-foreground">{startDate ? "Sin turno programado en esa fecha." : "Selecciona la fecha inicial."}</p>
                )}
              </div>
            </fieldset>
          )}
          ...
          {motivoExento && (
            <div className="rounded-md border border-amber-300 bg-amber-50/60 p-3 dark:bg-amber-950/10">
              ...
              <p className="mt-1 text-xs text-amber-700/90">
                ...
                {" "}Este tipo de permiso no se descuenta del límite mensual, pero debe contar con evidencia.
              </p>
              <div className="mt-2 flex items-center gap-2">
                <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" hidden onChange={(e) => seleccionarSoporte(e.target.files?.[0] ?? null)} />
                <Button type="button" size="sm" variant="outline" disabled={subiendoSoporte} onClick={() => fileRef.current?.click()}>
                  <Paperclip className="mr-1.5 h-4 w-4" /> {subiendoSoporte ? "Subiendo…" : soporte ? "Reemplazar soporte" : "Adjuntar evidencia"}
                </Button>
                {soporte && (
                  <span className="flex items-center gap-1 rounded bg-emerald-100 px-2 py-0.5 text-[11px] text-emerald-700">
                    ...
                    <button type="button" onClick={quitarSoporte} className="text-rose-600"><X className="h-3 w-3" /></button>
                  </span>
                )}
              </div>
            </div>
          ...
          {reqReemplazo && (
            <fieldset className="rounded-md border p-3">
              <legend className="px-1 text-xs font-semibold uppercase text-muted-foreground">Reemplazo / cobertura</legend>
              <div className="grid grid-cols-2 gap-3">
                <div>
                ...
                <div><Label className="text-xs">Cargo del reemplazo</Label><Input value={reempCargo} readOnly className="bg-muted/40" placeholder="Automático" /></div>
                {!esCambio && (
                  <>
                    <div><Label className="text-xs">Fecha que va a cubrir</Label><Input value={startDate} readOnly className="bg-muted/40" /></div>
                    <div>
                      <Label className="text-xs">Turno / horario a cubrir</Label>
                      <div className="flex h-9 items-center gap-2 rounded-md border bg-muted/40 px-2 text-xs">
                        {turnoSolInfo ? (
                          <><span className="font-semibold">{turnoSolInfo.name}</span><span className="text-muted-foreground">{turnoSolInfo.horario}</span></>
          ...
          {!esCambio && recupera && (
            <fieldset className="rounded-md border border-emerald-300 bg-emerald-50/40 p-3 dark:bg-emerald-950/10">
              ...
              <label className="mb-3 flex items-center gap-2 text-xs">
                <Checkbox checked={fraccionado} onCheckedChange={(v) => {
                  const on = !!v; setFraccionado(on);
                  setFracciones((prev) => prev.length === 0 ? [nuevaFraccion(1)] : prev);
                }} />
              ...
              <div className="space-y-3">
                {fracciones.map((f, i) => (
                  <FraccionEditor
                    ...
                    onChange={(nf) => setFracciones((prev) => prev.map((x, j) => (j === i ? nf : x)))}
                    onRemove={fraccionado && fracciones.length > 1 ? () => setFracciones((prev) => prev.filter((_, j) => j !== i)) : undefined}
                  />
                ))}
              </div>

              {fraccionado && (
                <Button type="button" size="sm" variant="outline" className="mt-3" onClick={() => setFracciones((prev) => [...prev, nuevaFraccion(prev.length + 1)])}>
                  <Plus className="mr-1.5 h-4 w-4" /> Agregar fracción
                </Button>
              )}

              <p className="mt-2 text-[11px] text-muted-foreground">
                El turno se toma automáticamente del Cuadro de Turno del funcionario en la fecha indicada.
              </p>
            </fieldset>
          )}
          ...
          {!esCambio && (recupera || minutosSolicitados > 0) && (
            <div className="grid grid-cols-2 gap-2 rounded-md bg-muted/50 p-3 text-xs sm:grid-cols-4">
              <div><p className="text-muted-foreground">Horas solicitadas</p><p className="font-bold">{minutosAHoras(minutosSolicitados)}</p></div>
              <div><p className="text-muted-foreground">Programadas devolución</p><p className="font-bold">{minutosAHoras(minutosProgramados)}</p></div>
              <div><p className="text-muted-foreground">Verificadas</p><p className="font-bold">0 h</p></div>
              <div><p className="text-muted-foreground">Saldo pendiente</p><p className="font-bold">{minutosAHoras(saldoMin)}</p></div>
              {descuadre && (
                <p className="col-span-2 text-amber-600 sm:col-span-4">
                  Las horas programadas ({minutosAHoras(minutosProgramados)}) no coinciden con las solicitadas ({minutosAHoras(minutosSolicitados)}). Justifica la diferencia para continuar.
                </p>
              )}
            </div>
          )}
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={saving || subiendoSoporte}>{saving ? "Enviando…" : "Solicitar"}</Button>
        </DialogFooter>
      </DialogContent>

      {/* Modal límite mensual / autorización excepcional */}
      <Dialog open={limiteOpen} onOpenChange={(v) => { if (!v) { setLimiteOpen(false); setExcMode("aviso"); } }}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Límite mensual alcanzado</DialogTitle></DialogHeader>
          {excMode === "aviso" ? (
            <div className="space-y-3 text-sm">
              <p>Ya has utilizado el límite mensual de {LIMITE_MENSUAL} participaciones en permisos o cambios de turno autorizado por la institución.</p>
              <p className="text-muted-foreground">Para presentar una solicitud adicional debes solicitar autorización previa a coordinación.</p>
              {uso && (
                <div className="grid grid-cols-3 gap-2 rounded-md bg-muted/50 p-2 text-xs">
                  <div><p className="text-muted-foreground">Solicitudes</p><p className="font-bold">{uso.solicitudes}</p></div>
                  <div><p className="text-muted-foreground">Coberturas</p><p className="font-bold">{uso.coberturas}</p></div>
                  <div><p className="text-muted-foreground">Total</p><p className="font-bold">{uso.total} / {LIMITE_MENSUAL}</p></div>
                </div>
              )}
              <p className="text-sm font-medium">¿Deseas enviar una solicitud a coordinación para que se autorice un permiso o cambio adicional?</p>
              <DialogFooter>
                <Button variant="outline" onClick={() => setLimiteOpen(false)}>Cancelar</Button>
                <Button onClick={() => setExcMode("motivo")}>Solicitar autorización excepcional</Button>
              </DialogFooter>
            </div>
          ) : (
            <div className="space-y-3 text-sm">
              <div>
                <Label className="text-xs">Motivo por el cual requiere superar el límite mensual</Label>
                <Textarea value={excMotivo} onChange={(e) => setExcMotivo(e.target.value)} rows={3} />
              </div>
              <p className="text-xs text-muted-foreground">
                Esta solicitud no crea automáticamente el permiso o cambio de turno. Si coordinación la aprueba, se habilitará una única solicitud adicional para el mes seleccionado. Si coordinación la niega, no podrás registrar otra solicitud no exceptuada durante ese mes.
              </p>
              <DialogFooter>
                <Button variant="outline" onClick={() => setExcMode("aviso")}>Volver</Button>
                <Button onClick={enviarExcepcion} disabled={excSaving}>{excSaving ? "Enviando…" : "Enviar a coordinación"}</Button>
              </DialogFooter>
            </div>
          )}
        </DialogContent>
