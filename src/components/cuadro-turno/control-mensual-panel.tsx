...
interface Miembro { nombre: string; cargo: string | null; userId: string | null; }
interface ExcRow {
  id: string; user_id: string; user_name: string | null; user_role: string | null;
  year: number; month: number; request_type: string | null; reason: string;
  counts: Record<string, number> | null; status: string; usage_status: string;
  used_request_id: string | null; reviewed_by_name: string | null; reviewed_at: string | null;
  created_at: string;
}
...
export function ControlMensualPanel({ soloUsuario }: { soloUsuario?: boolean }) {
  ...
  const { data: requests = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("shift_requests")
        .select("requester_id, replacement_user_id, status, reason_type, is_limit_exempt, request_type, start_date, original_shift_date, created_at, requester_name, requester_role");
      return data ?? [];
    },
  });
  ...
  const { data: miembros = [] } = useQuery({
    ...
    queryFn: async (): Promise<Miembro[]> => {
      ...
      (profs ?? []).forEach((p) => { const n = (p.nombre || "").trim(); if (n && !map.has(n)) map.set(n, { nombre: n, cargo: p.cargo, userId: p.user_id }); });
      (members ?? []).forEach((m) => { const n = (m.full_name || "").trim(); if (n && !map.has(n)) map.set(n, { nombre: n, cargo: m.role_name, userId: m.user_id }); });
    ...
    return base
      ...
      .map((m) => {
        const uso = m.userId
          ...
          : { solicitudes: 0, coberturas: 0, pendientes: 0, aprobadas: 0, exentos: 0, total: 0, disponible: LIMITE_MENSUAL };
        const excAprob = excepciones.filter((e) => e.user_id === m.userId && e.year === year && e.month === month && e.status === "APROBADA");
  ...
  }, [miembros, requests, excepciones, year, month, fNombre, fCargo, fEstado, soloUsuario, user?.id]);

  const excPendientes = useMemo(
    () => excepciones.filter((e) => e.status === "PENDIENTE" && (soloUsuario ? e.user_id === user?.id : true)),
  ...
  const resolverExc = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("shift_monthly_exceptions").update({
        status: excAccion,
        reviewed_by: user.id, reviewed_by_name: prof?.nombre || null,
        reviewed_at: new Date().toISOString(), review_reason: excRazon || null,
        usage_status: excAccion === "APROBADA" ? "DISPONIBLE" : "VENCIDA",
      }).eq("id", excSel.id);
      if (error) throw error;
      registrarAuditoria({ data: { accion: excAccion === "APROBADA" ? "EXCEPCION_APROBADA" : "EXCEPCION_NEGADA", modulo: "cuadro_turno", tabla: "shift_monthly_exceptions", registroId: excSel.id, resultado: "exito" } }).catch(() => {});
      toast.success(excAccion === "APROBADA" ? "Excepción aprobada." : "Excepción negada.");
      setExcSel(null); setExcAccion(null); setExcRazon("");
      qc.invalidateQueries({ queryKey: ["monthly-exceptions"] });
    } catch (e) { console.error(e); toast.error("No se pudo procesar la excepción."); }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="flex flex-wrap items-end gap-3 p-3">
        ...
        <div><Label className="text-xs">Mes</Label>
          <Select value={String(month)} onValueChange={(v) => setMonth(Number(v))}>
            ...
            <SelectContent>{MESES.map((mm, i) => <SelectItem key={mm} value={String(i + 1)}>{mm}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        {!soloUsuario && (
          <>
            <div><Label className="text-xs">Funcionario</Label><Input className="w-44" value={fNombre} onChange={(e) => setFNombre(e.target.value)} placeholder="Buscar…" /></div>
            <div><Label className="text-xs">Cargo</Label><Input className="w-40" value={fCargo} onChange={(e) => setFCargo(e.target.value)} placeholder="Buscar…" /></div>
          </>
        )}
      ...
      {excPendientes.length > 0 && (
        <Card className="p-3">
          <h4 className="mb-2 text-sm font-bold">Autorizaciones excepcionales pendientes · {excPendientes.length}</h4>
          <div className="space-y-2">
            {excPendientes.map((e) => (
              <div key={e.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-2 text-xs">
                <div className="min-w-0">
                  <p className="font-semibold">{e.user_name || "—"} <span className="text-muted-foreground">· {e.user_role || ""}</span></p>
                  <p className="text-muted-foreground">{MESES[e.month - 1]} {e.year} · {e.request_type} · Enviada {fmtFechaHora(e.created_at)}</p>
                  <p className="mt-0.5">Motivo: {e.reason}</p>
                </div>
                {!soloUsuario && (
                  <div className="flex gap-1">
                    <Button size="sm" onClick={() => { setExcSel(e); setExcAccion("APROBADA"); }}><ShieldCheck className="mr-1 h-3.5 w-3.5" />Aprobar</Button>
                    <Button size="sm" variant="destructive" onClick={() => { setExcSel(e); setExcAccion("NEGADA"); }}><ShieldX className="mr-1 h-3.5 w-3.5" />Negar</Button>
                  </div>
                )}
              </div>
            ))}
      ...
      <Card className="p-3">
        <h4 className="mb-2 text-sm font-bold">Control mensual de cambios y permisos · {MESES[month - 1]} {year}</h4>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              {filas.length === 0 && (
                <tr><td colSpan={10} className="py-4 text-center text-muted-foreground">Sin funcionarios para los filtros.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Dialog open={!!excSel} onOpenChange={(v) => { if (!v) { setExcSel(null); setExcAccion(null); setExcRazon(""); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{excAccion === "APROBADA" ? "Aprobar" : "Negar"} autorización excepcional</DialogTitle></DialogHeader>
          <div className="space-y-3 text-sm">
            <p className="text-xs text-muted-foreground">{excSel?.user_name} · {excSel && MESES[excSel.month - 1]} {excSel?.year}</p>
            <p className="text-xs">Motivo del funcionario: {excSel?.reason}</p>
            {excAccion === "APROBADA" ? (
              <p className="text-xs text-muted-foreground">Se habilitará una única solicitud adicional para ese mes. La autorización es de un solo uso y no transferible.</p>
            ) : (
              <div><Label className="text-xs">Razón de la negación (obligatoria)</Label><Textarea value={excRazon} onChange={(e) => setExcRazon(e.target.value)} rows={2} /></div>
            )}
          </div>
          <DialogFooter>
